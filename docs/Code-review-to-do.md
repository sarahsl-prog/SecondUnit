...
- [ ] **GCP dry-run flag.** Add a config flag (default: dry-run/off) gating `GCPComputeClient.start_preemptible_instances` from actually creating instances; document how to flip it on for demo day.
- [ ] **Dispatcher human fallback on zero channels.** When `channels` ends up empty in `DispatcherAgent.notify()`, don't just log — write to a fallback surface (e.g. a local file/queue an operator can check, or escalate via whatever channel *is* guaranteed, like stdout structured log at ERROR severity with a distinct event name) so a fully-silent remediation is never possible.
