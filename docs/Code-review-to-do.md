...
- [x] **#19 — `MetricsEmitter`/`LogEmitter` are no-op stubs.** Confirmed — both methods are `pass`. Same scope question as #18.
...
- [x] **5th failure scenario test.** Confirm the 5th scenario name in `simulator/failures.py` (review names `gpu_memory_exhaustion`, `corrupt_scene_file`, `network_timeout`, `license_failure` — one more exists per "five scenarios" in the review) and add its Pathologist classification test alongside #10's fix.
